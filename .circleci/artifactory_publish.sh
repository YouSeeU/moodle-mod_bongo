...
./jfrog rt u --build-name=${CIRCLE_PROJECT_REPONAME} --build-number=${CIRCLE_BUILD_NUM} --flat=true --recursive=false \*.tgz ${ARTIFACTORY_REPO}/${ARTIFACTORY_ORG}/${MOODLE_MODULE}/
